...
func (r *remoteKeySet) verify(ctx context.Context, jws *jose.JSONWebSignature) ([]byte, error) {
	...
	if !r.now().Add(keysExpiryDelta).After(expiry) {
		...
		return nil, errors.New("Keys haven't expired failed to verify id token signature")
	}
	...
	for _, key := range keys {
        fmt.Printf("keyID %s   key.KeyID %s", keyID, key.KeyID)
		if keyID == "" || key.KeyID == keyID {
			if payload, err := jws.Verify(&key); err == nil {
				return payload, nil
			} else  {
			return nil, fmt.Errorf("errrrrrrr: %s", err.Error())
			}
		}
	}
	return nil, errors.New("ohter failed to verify id token signature")
}
